...
var spark = module.exports = Base.extend({
  ...
  _describePath : function (data, paddingLeft, paddingTop, scale, ref) {
    ref = ref || 0;
    var self = this;
    var pathToken = svgPath.describeAttributeD(data.data, paddingLeft, paddingTop, scale);
    ...
    if (data.fill && scale.fill) {
      paths.push(self.make('path', {
        d: pathToken + svgPath.describeCloseAttributeD(data.data, paddingLeft, paddingTop, scale),
        stroke: 'none',
        'stroke-width': '2',
        'stroke-linejoin': 'round',
        'stroke-linecap': 'round',
