...
var spark = module.exports = Base.extend({
  ...
  _startCycle: function () {
    ...
    scale.tickSize = self._sigFigs(((chart.width - paddingX * 2) / (scale.len - 1)),8);
    ...
    for (var i = 0; i < numArr.length; i++) {
        if (i === 0) {
          // X Y
            ...
            pathToken += ' L '+ ((tickSize * i) + paddingX) + ' ' + (height - (numArr[i] * heightRatio) - paddingY);
        }
    }
