...
var spark = module.exports = Base.extend({
  ...
  _generate: function () {
    ...
    if (opts.yAxis) {
      console.log(scale);
      // svg = self.append(svg, label.describes(scale, opts.yAxis));
      // paddingX = scale.paddingX;
      // paddingY = scale.paddingY;
    }
    ...
    for (var x = 0; x < data.length; x++) {
        ...
        svg = self.append(svg,
          self.append(g, self._describePath(data[x], paddingX, paddingY, opts))
          );
    }
    // add to element;
