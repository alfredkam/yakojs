...
var spark = module.exports = Base.extend({
  ...
  _generate: function (reRender) {
    ...
    // add to element;
    var result = self._compile(self.element, svg, reRender);
  ...
  _describePath : function (data, paddingX, paddingY, opts) {
      var pathToken = this._describeAttributeD(data.data, paddingX, paddingY, opts);
      ...
      return pathNode + this._make('path', {
        d: pathToken + this._describeCloseAttributeD(data.data, paddingX, paddingY, opts),
        stroke: 'none',
        'stroke-width': '2',
        'stroke-linejoin': 'round',
        'stroke-linecap': 'round',
