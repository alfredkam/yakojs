...
module.exports = utils = Class.extend({
   ...
   */
  _defineBaseScaleProperties: function (scale, chart, properties) {
    var self = this;
    self._extend(scale, chart);
    self._extend(scale, properties);
    self._getRatio(scale);
  },
  _lifeCycleManager: function (scale, describe) {
    var self = this;
    // make the obj's shallow properties immutable
    // TODO:: deep freeze when obj has more then one child
    var properties = this.preRender(Object.freeze(self._deepCopy(scale)));
    // properties will except
    // append
    // prepend
    // scale
    self._extend(scale, properties.scale);
    self._getRatio(scale);
